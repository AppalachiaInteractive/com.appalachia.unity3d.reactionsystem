...
namespace Appalachia.ReactionSystem.TouchBend.Data
{
    ...
    public class TouchBendQuad : MonoBehaviour
    {
        ...
        public void InitializeCamera()
        {
            ...
            if (info == null)
            {
                var directory = GSC.TOUCHBEND.PATH_INTERACTION_MASKS;
                var runtimeName = $"{gameObject.name}_INTERACTION-INFO";

                info = TouchBendComponentInfo.LoadOrCreateNew(directory, runtimeName);
            }
        }
        ...
        private void SaveTextureAssetToFile()
        {
            try
            {
                var fileName = info.texture.name;

                if (fileName.EndsWith(".png"))
                {
                    fileName = fileName.Replace(".png", string.Empty);
                    info.texture.name = fileName;
                }

                var targetSavePath = $"{GSC.TOUCHBEND.PATH_INTERACTION_MASKS}{fileName}.png";
                ...
                info.texture = AssetDatabase.LoadAssetAtPath<Texture2D>(targetSavePath);
